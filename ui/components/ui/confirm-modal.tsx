...
export default function ConfirmModal({ open, title, message, onCancel, onConfirm }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(14,165,233,0.12)' }}>
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button onClick={onConfirm} className="px-3 py-2 rounded bg-[var(--color-error)] text-white">Delete</button>
        </div>
      </div>
    </div>
  );
